...
import 'package:wafy/features/auth/presentation/pages/widgets/loading_widget.dart';
...
class FormAnimationWidget extends GetView<LoginController> {
  const FormAnimationWidget({super.key});
    ...
    return TweenAnimationBuilder<Offset>(
      ...
      builder: (context, value, child) {
        return Transform.translate(
          offset: value,
          child: Opacity(
            opacity: value.dy == 0 ? 1.0 : 0.0,
            child: Obx(() {
              return controller.state.when(
                initial: () => LoginFormWidget(),
                loading: () => const LoadingWidget(),
                success: (user) => LoginFormWidget(),
                error: (message) => LoginFormWidget(errorMessage: message),
              );
            }),
          ),
        );
